...
public class PlanilhaApuracaoIssAllReport extends BasicPdfReport {
	...
	public PlanilhaApuracaoIssAllReport(List<RelatorioApuracaoIssAllDTO> relatorioApuracaoIssDTO, Boolean aberturaConta) {
		super();
...
//		montarTotalGeralFinal(document, relatorioApuracaoIssAllDTO);
	...
	private Table montarCabecalho() throws BadElementException, DocumentException {
		Table tableCabecalho = new Table(2);
		tableCabecalho.setWidth(100);
		tableCabecalho.setBorder(Rectangle.NO_BORDER);

		Cell celNomePlanilha = new Cell(new Chunk("Planilha de Apuração do I.S.S.", FONT_NORMAL));
		celNomePlanilha.setBorder(Cell.NO_BORDER);
		celNomePlanilha.setColspan(2);
		tableCabecalho.addCell(celNomePlanilha);

		Cell celUnidadeOrganizacional = new Cell(new Chunk("BANCO BRADESCO S.A.", FONT_NORMAL));
		...
		agencia.append(String.valueOf(relatorioApuracaoIssOneDTO.getCodAgenciaDev()));
		agencia.append("/");
		agencia.append(relatorioApuracaoIssOneDTO.getDescAgenciaDev());
		...
		Cell celEstado = new Cell(new Chunk("ESTADO: " + relatorioApuracaoIssOneDTO.getSiglaEstado(), FONT_NORMAL));
		celEstado.setBorder(Cell.NO_BORDER);
		celEstado.setHorizontalAlignment(Cell.ALIGN_RIGHT);
		tableCabecalho.addCell(celEstado);

		Cell celCnpj = new Cell(new Chunk("CNPJ: " + NumberUtils.formatCnpj(relatorioApuracaoIssOneDTO.getCnpj()), FONT_NORMAL));
		celCnpj.setBorder(Cell.NO_BORDER);
		tableCabecalho.addCell(celCnpj);

		Cell celInscricaoMunicipal = new Cell(new Chunk("INSCRICAO MUNICIPAL: " + relatorioApuracaoIssOneDTO.getInscricaoMunicipal(), FONT_NORMAL));
		...
		endereco.append(relatorioApuracaoIssOneDTO.getEndereco());
		...
		Cell celCep = new Cell(new Chunk("CEP: " + relatorioApuracaoIssOneDTO.getCepFormatado(), FONT_NORMAL));
		...
		centralizadora.append(GissUtil.formatNumber(String.valueOf(relatorioApuracaoIssOneDTO.getCodAgenciaCentralizadora()), 5));
		centralizadora.append("-");
		centralizadora.append(relatorioApuracaoIssOneDTO.getDigitoAgenciaCentralizadora());
		centralizadora.append("/");
		centralizadora.append(relatorioApuracaoIssOneDTO.getDescAgenciaCentralizadora());
		Cell celCentralizadora = new Cell(new Chunk(centralizadora.toString(), FONT_NORMAL));
		...
		municipio.append(relatorioApuracaoIssOneDTO.getCodigoMunicipio());
		municipio.append("/");
		municipio.append(relatorioApuracaoIssOneDTO.getDigitoMunicipio());
		Cell celCodMunic = new Cell(new Chunk("COD.MUNIC.: " + municipio.toString(), FONT_NORMAL));
		...
		competencia.append(relatorioApuracaoIssOneDTO.getMesCompetencia());
		competencia.append("/");
		competencia.append(relatorioApuracaoIssOneDTO.getAnoCompetencia());
		Cell celCompetencia = new Cell(new Chunk("COMPETENCIA: " + competencia.toString(), FONT_NORMAL));
		celCompetencia.setBorder(Cell.NO_BORDER);
		tableCabecalho.addCell(celCompetencia);

		Cell celVencimento = new Cell(new Chunk("VENCIMENTO: " + relatorioApuracaoIssOneDTO.getDiaVencimento(), FONT_NORMAL));
	...
	private Table montarDescricaoDemonstrativo() throws BadElementException, DocumentException {
		Table table = new Table(1);
		table.setWidth(100);
		table.setBorder(Rectangle.NO_BORDER);

		Cell celTexto1 = new Cell(new Chunk("DEMONSTRATIVO DAS RECEITAS, PARA EFEITO DE RECOLHIMENTO DO ISSQN, A  SER EFETUADO  NO  VENCIMENTO. O LANCAMENTO", FONT_NORMAL));
		celTexto1.setBorder(Cell.NO_BORDER);
		celTexto1.setHorizontalAlignment(Cell.ALIGN_JUSTIFIED_ALL);
		table.addCell(celTexto1);

		Cell celTexto2 = new Cell(new Chunk("CONTABIL FOI FEITO A DEBITO DA CONTA (55.89.1/9) NA AGENCIA/DEPARTAMENTO E A CREDITO DA CONTA", FONT_NORMAL));
		celTexto2.setBorder(Cell.NO_BORDER);
		celTexto2.setHorizontalAlignment(Cell.ALIGN_JUSTIFIED_ALL);
		table.addCell(celTexto2);

		Cell celTexto3 = new Cell(new Chunk("(17.50.1/9), NA AGENCIA/DEPARTAMENTO CENTRALIZADOR, QUE E O RESPONSAVEL PELO RECOLHIMENTO E PELA", FONT_NORMAL));
		celTexto3.setBorder(Cell.NO_BORDER);
		celTexto3.setHorizontalAlignment(Cell.ALIGN_JUSTIFIED_ALL);
		table.addCell(celTexto3);

		Cell celTexto4 = new Cell(new Chunk("GUARDA DOS DOCUMENTOS POR 11 (ONZE) ANOS.", FONT_NORMAL));
		celTexto4.setBorder(Cell.NO_BORDER);
		celTexto4.setHorizontalAlignment(Cell.ALIGN_JUSTIFIED);
		table.addCell(celTexto4);

		Cell celTexto5 = new Cell(new Chunk("(NO ATO DO RECOLHIMENTO DEBITAR A CONTA (17.50 - 1/9) QUE DEVERA SER ZERADA).", FONT_NORMAL));
	...
	private void montarLista(Document document,RelatorioApuracaoIssAllDTO relatorioApuracaoIssDTO) throws BadElementException, DocumentException {
		
		// Conteudo Novo
		// Lisca do Conteudo Carregado
		List<OcorrenciaApuracaoIssDTO> lst = relatorioApuracaoIssOneDTO.getApuracoes();
		
		if (lst.size() > 0) {
			if (lst.get(0).getCodigoGrupo() <= 0 || lst.get(0).getCodigoSubgrupo() <= 0) {
				razaoExiste = false;
			}
			else{
				razaoExiste = true;
		...
		Cell celLinhaBranco = new Cell(Chunk.NEWLINE);
		celLinhaBranco.setBorder(Cell.NO_BORDER);
		celLinhaBranco.setColspan(nroColunas);

		Cell celTituloTipoServico = new Cell(new Chunk("TIPO DE SERVICO", FONT_NORMAL));
		celTituloTipoServico.setBorder(Cell.NO_BORDER);
		celTituloTipoServico.setColspan(nroColunas);
		...
		List<OcorrenciaApuracaoIssDTO> lstComp = null;
		while (it.hasNext()) {
			OcorrenciaApuracaoIssDTO itemTemp = (OcorrenciaApuracaoIssDTO) it.next();

			if (primeiro) {
				// Lista Temporaria armazenando os Itens da Lista
				lstComp = new ArrayList<OcorrenciaApuracaoIssDTO>();
				lstIndex.add(itemTemp.getCompetencia());
				primeiro = false;
			}

			if (lstIndex.contains(itemTemp.getCompetencia())) {
				lstComp.add(itemTemp);
			} else {
				lstOb.add(lstComp);
				lstIndex.add(itemTemp.getCompetencia());
				lstComp = new ArrayList<OcorrenciaApuracaoIssDTO>();
		...
		for (int i = 0; i < lstOb.size(); i++) {
			List<OcorrenciaApuracaoIssDTO> lista = (List<OcorrenciaApuracaoIssDTO>) lstOb.get(i);
			StringBuilder descricaoTipoServico;

			for (int j = 0; j < lista.size(); j++) {
				OcorrenciaApuracaoIssDTO item = lista.get(j);
				if (tipoServicoNovo) {
					descricaoTipoServico = new StringBuilder();
					descricaoTipoServico.append(item.getCodigoTipoServico());
					descricaoTipoServico.append("/");
					descricaoTipoServico.append(item.getDescricaoTipoServico());
					Cell celTipoServico = new Cell(new Chunk(descricaoTipoServico.toString(), FONT_NORMAL));
					celTipoServico.setBorder(Cell.NO_BORDER);
					celTipoServico.setColspan(nroColunas);
					table.addCell(celTipoServico);
					tipoServicoNovo = false;
					
				}
				if (!item.isTotal()) {
					montarResultadoTabela(table, item);
				} else {
					montarTotalTabela(table, item);
				}
				tipoServicoNovo = item.isTotal();
			}
			if (!(i == (lstOb.size() - 1))) {
				//document.add(Chunk.NEXTPAGE);
			}
		...
		// TODO: remover apos conclusao do modulo acima que deve estar funcionando
		// for (int i = 0; i < relatorioApuracaoIssDTO.getApuracoes().size(); i++) {
		// OcorrenciaApuracaoIssDTO apuracao = relatorioApuracaoIssDTO.getApuracoes().get(i);
		// if (tipoServicoNovo) {
		// StringBuilder descricaoTipoServico = new StringBuilder();
		// descricaoTipoServico.append(apuracao.getCodigoTipoServico());
		// descricaoTipoServico.append("/");
		// descricaoTipoServico.append(apuracao.getDescricaoTipoServico());
		// Cell celTipoServico = new Cell(new Chunk(descricaoTipoServico.toString(), FONT_NORMAL));
		// celTipoServico.setBorder(Cell.NO_BORDER);
		// celTipoServico.setColspan(6);
		// table.addCell(celTipoServico);
		//
		// tipoServicoNovo = false;
		// }
		//
		// if (!apuracao.isTotal()) {
		// montarResultadoTabela(table, apuracao);
		// } else {
		// montarTotalTabela(table, apuracao);
		// }
		//
		...
		table.addCell(celLinhaBranco);

		montarTotalGeral(table, relatorioApuracaoIssDTO);
	...
	private void montarResultadoTabela(Table table, OcorrenciaApuracaoIssDTO apuracao) throws BadElementException {
		...
		Cell celConta = new Cell(new Chunk(GissUtil.mascaraContaContabil(apuracao.getCosif().toString(), apuracao.getRazao()), FONT_NORMAL));
		...
		Cell celIss = new Cell(new Chunk(NumberUtils.format(apuracao.getValorTributo()), FONT_NORMAL));
		celIss.setBorder(Cell.NO_BORDER);
		celIss.setHorizontalAlignment(Cell.ALIGN_RIGHT);
		table.addCell(celIss);
	}

	private void montarTotalTabela(Table table, OcorrenciaApuracaoIssDTO apuracao) throws BadElementException {
		...
		Cell celTotalReceita = new Cell(new Chunk(NumberUtils.format(apuracao.getValorReceita()), FONT_NORMAL));
		...
		Cell celTotalIss = new Cell(new Chunk(NumberUtils.format(apuracao.getValorTributo()), FONT_NORMAL));
	...
	private void montarCabecalhoTabela(Table table) throws BadElementException {
	...
	private void montarTotalGeral(Table table, RelatorioApuracaoIssAllDTO relatorioApuracaoIssDTO) throws BadElementException {
		...
		if(aberturaConta){
			...
			Cell celValorRedutor = new Cell(new Chunk(NumberUtils.format(relatorioApuracaoIssDTO.getValorRedutorTotal()), FONT_NORMAL));
			...
			Cell celValorTotalReceita = new Cell(new Chunk(NumberUtils.format(relatorioApuracaoIssDTO.getValorTotalReceitaTotal()), FONT_NORMAL));
			...
			Cell celValorTotalIss = new Cell(new Chunk(NumberUtils.format(relatorioApuracaoIssDTO.getValorTotalIssTotal()), FONT_NORMAL));
	...
	private void montarTotalGeralFinal(Document document, RelatorioApuracaoIssAllDTO relatorioApuracaoIssAllDTO) throws BadElementException, DocumentException {

		Table table = new Table(6);
		table.setWidth(100);
		table.setWidths(new int[] { 14, 40, 10, 14, 10, 12 });
		table.setBorder(Rectangle.NO_BORDER);

		Cell celLinhaBranco = new Cell(Chunk.NEWLINE);
		celLinhaBranco.setBorder(Cell.NO_BORDER);
		celLinhaBranco.setColspan(6);

		Cell celRedutor = new Cell(new Chunk("REDUTOR GERAL...................................................", FONT_NORMAL));
		celRedutor.setBorder(Cell.NO_BORDER);
		celRedutor.setHorizontalAlignment(Cell.ALIGN_LEFT);
		celRedutor.setColspan(3);
		table.addCell(celRedutor);
		...
		Cell celValorRedutor = new Cell(new Chunk(NumberUtils.format(relatorioApuracaoIssAllDTO.getValorRedutorTotal()), FONT_NORMAL));
		celValorRedutor.setBorder(Cell.NO_BORDER);
		celValorRedutor.setHorizontalAlignment(Cell.ALIGN_RIGHT);
		table.addCell(celValorRedutor);

		Cell celTotalAgencia = new Cell(new Chunk("TOTAL GERAL.....................................................", FONT_NORMAL));
		celTotalAgencia.setBorder(Cell.NO_BORDER);
		celTotalAgencia.setHorizontalAlignment(Cell.ALIGN_LEFT);
		celTotalAgencia.setColspan(3);
		table.addCell(celTotalAgencia);

		Cell celValorTotalReceita = new Cell(new Chunk(NumberUtils.format(relatorioApuracaoIssAllDTO.getValorTotalReceitaTotal()), FONT_NORMAL));
		celValorTotalReceita.setBorder(Cell.NO_BORDER);
		celValorTotalReceita.setHorizontalAlignment(Cell.ALIGN_RIGHT);
		table.addCell(celValorTotalReceita);

		Cell celValorTotalIss = new Cell(new Chunk(NumberUtils.format(relatorioApuracaoIssAllDTO.getValorTotalIssTotal()), FONT_NORMAL));
		celValorTotalIss.setBorder(Cell.NO_BORDER);
		celValorTotalIss.setHorizontalAlignment(Cell.ALIGN_RIGHT);
		celValorTotalIss.setColspan(2);
		table.addCell(celValorTotalIss);
	...
	private Table montarRodape() throws BadElementException, DocumentException {
		...
		Cell celTitulo = new Cell(new Chunk("ASSUMO INTEIRA RESPONSABILIDADE PELA EXATIDAO DOS DADOS FORNECIDOS:", FONT_NORMAL));
	...
	private Image getImagemRelatorio() {
		try {
			...
			if (caminhoImagem == null) {
				throw new ReportException("Não encontrou a imagem no caminho: " + caminho);
			}
