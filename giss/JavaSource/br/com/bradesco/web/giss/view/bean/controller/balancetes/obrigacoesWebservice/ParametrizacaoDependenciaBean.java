...
public class ParametrizacaoDependenciaBean implements Serializable,
		BuscaMunicipioCallback {

	// Serial Version
	private static final long serialVersionUID = 2968893363701127901L;
	...
	private List<SelectItem> listaEstados = new ArrayList<SelectItem>(); // lista do combo de estados
	private List<SelectItem> selectItemLista = new ArrayList<SelectItem>(); 
	private List<SelectItem> selectItemListaCont = new ArrayList<SelectItem>();
	private List<DependenciasNovasDTO> listaDependenciasNovas = new ArrayList<DependenciasNovasDTO>(); // lista de dependencias novas
	...
	private boolean mostraDependencias; // mostra ou não a lista das
										// dependências novas
	...
	private String codListaPesquisarMunicipio; // código do municipio
												// selecionado
	...
	private String codigoMunicipio; // variavel auxiliar para guardar código do
									// munipio
	private String codigoEstado; // variavel auxiliar para guardar codigo do
									// estado
	private String nomeMunicipio; // variavel auxiliar para guardar nome do
									// munipio
	...
	private String codigoAgenciaIncorpDepto; // codigo agencia incorporadora
												// departamento
	private String nomeAgenciaIncorpDepto; // nome agencia incorporadora
											// departamento
	...
	private String codigoMunicipioManter; // variavel auxiliar para lista
											// municipio
	...
	IListarEmpresaService listarEmpresaServiceImpl = (IListarEmpresaService) FacesUtils
			.getManagedBean("listarEmpresaServiceImpl");
	...
	IObterDescDependenciaService obterDescDependenciaServiceImpl = (IObterDescDependenciaService) FacesUtils
			.getManagedBean("obterDescDependenciaServiceImpl");
	...
	public String inicializaManterDependencia() {
		...
		return "manterDependenciaParametrizacao";
	}
	...
	public String pesquisarMunicipioIncorpora() {
		if (buscaMunicipio.getCodigo() != null
				&& buscaMunicipio.getCodigo() != 0) {
			buscaMunicipioDepNova();
			codMunicipioIncorp = buscaMunicipio.getCodigo();
			listarAgenciasPorMunicipio(String.valueOf(buscaMunicipio
					.getCodigo()));
			return "prosseguir_agencias_dependencias_novas";
		} else if (buscaMunicipio.getDescricao() != null
				&& !buscaMunicipio.getDescricao().equals("")) {
			buscaMunicipioDepNova();
			return "";
			...
			BradescoFacesUtils.addInfoModalMessage(
					"Selecione o estado ou preencha o municipio.", false);
			return "";
		}
	}

	public String pesquisarMunicipioIncorporaManter() {
		if (buscaMunicipioIncorp.getCodigo() != null
				&& buscaMunicipioIncorp.getCodigo() != 0) {
			buscaMunicipioIncorp();
			codMunicipioIncorp = buscaMunicipioIncorp.getCodigo();
			listarAgenciasPorMunicipio(String.valueOf(buscaMunicipioIncorp
					.getCodigo()));
			return "prosseguir_agencias_manter_dependencias";
		} else if (buscaMunicipioIncorp.getDescricao() != null
				&& !buscaMunicipioIncorp.getDescricao().equals("")) {
			buscaMunicipioIncorp();
			return "";
		} else if (!estadoResultDepNovasMun.equals("")) {
			listaMunicipiosAgIncorpora.clear();
			listaMunicipiosAgIncorpora = listarMunicipios("M",
					estadoResultDepNovasMun);
			mostraMunicipiosIncorpora = true;
			return "";
		} else {
			BradescoFacesUtils.addInfoModalMessage(
	...
	public void buscaMunicipio(String parametroEstado) {
		if (!tipo.equals("manterIncorp")) {
			...
			for (int i = 0; i < listaDosEstados.size(); i++) {
				if (parametroEstado.equals(listaDosEstados.get(i).getCodigo())) {
	...
	public void processa() {
		if (tipo.equals("depNova")) {
			estadoResultDepNovasMun = GissUtil
					.procuraCodigoUnidadeFederativaPorSigla(listaDosEstados,
							buscaMunicipio.getUf());
		} else if (tipo.equals("manter")) {
			estadoFiltro = GissUtil.procuraCodigoUnidadeFederativaPorSigla(
					listaDosEstados, buscaMunicipio.getUf());
		} else if (tipo.equals("manterIncorp")) {
			estadoResultDepNovasMun = GissUtil
					.procuraCodigoUnidadeFederativaPorSigla(listaDosEstados,
							buscaMunicipioIncorp.getUf());
		}
	}
	...
	public void carregarDependencia(String empresa, Integer codDependencia) {
		if (codDependencia != null && codDependencia != 0) {
			dep = obterDescDependenciaServiceImpl.obterDescDependencia(
					Long.valueOf(empresa), codDependencia);
		} else {
			BradescoFacesUtils.addInfoModalMessage(
					"Favor preencher a Dependencia.", false);
			return;
		}
	}
	...
	private void listarDependenciasNovas(String empresa, Integer codDependencia) {
		...
		try {
			...
			listaDependenciasNovas = dependenciaImpl.listarDependenciasNovas(
					empresa != null ? Long.valueOf(empresa) : 0,
					codDependencia != null ? codDependencia : 0);
	...
	public void paginarDependenciasNovas(ActionEvent event) {
		...
		listarDependenciasNovas(
				codDepFiltro != null && codDepFiltro != 0 ? indiceComboEmpresaFiltro
						: "0",
				codDepFiltro != null && codDepFiltro != 0 ? codDepFiltro : 0);
	...
	public String prosseguirDependenciasNovas() {
		...
		DependenciasNovasDTO dependenciasNovasDTO = listaDependenciasNovas
	...
	public boolean consultaInformacoesDependenciaUnidadeOrganizacional(
			String codDependencia) {
		try {
			DependenciaDTO dependenciaDTO = dependenciaImpl
					.consultarInformacoesDependenciaUnidOrgan(codDependencia);

			codigoPessoaJuridica = dependenciaDTO.getCodigoPessoaJuridica();
			numeroUnidadeOrganizacional = dependenciaDTO
					.getSeqUnidadeOrganizacional();
		} catch (PdcAdapterFunctionalException p) {
	...
	public void consultarInformacoesDependencia(
			DependeciasEncerradasDTO dependenciasDto) {
		try {

			DependenciaDTO dependenciaDTO = dependenciaImpl
					.consultarInformacoesDependencia(dependenciasDto);
			...
			codigoAgenciaIncorpDepto = dependenciaDTO
					.getCodAgenciaIncorporadoraDepartamento(); //
			nomeAgenciaIncorpDepto = dependenciaDTO
					.getDescAgenciaIncorporadoraDepartamento(); //
			estadoMunicIncorpDepto = dependenciaDTO
					.getEstadoMunicipioAgenIncorpDepto();
			codigoPessoaAgencIncorpDepto = dependenciaDTO
					.getCodigoPessoaAgencIncorpDepto();
			numeroSequencAgencIncorpDepto = dependenciaDTO
					.getNumeroSequencAgencIncorpDepto();
			estadoFiltro = GissUtil.procuraCodigoUnidadeFederativaPorSigla(
					listaDosEstados, estadoMunicIncorpDepto);
			codigoMunicipioManter = dependenciaDTO.getCodMunicipio().toString();
			codDepIncorpora = dependenciaDTO
					.getCodAgenciaIncorporadoraDepartamento() != null
					&& !dependenciaDTO.getCodAgenciaIncorporadoraDepartamento()
							.equals("") ? !dependenciaDTO
					.getCodAgenciaIncorporadoraDepartamento().equals("0") ? Integer
					.valueOf(dependenciaDTO
							.getCodAgenciaIncorporadoraDepartamento()) : null
					: null; //
			descDepIncorpora = dependenciaDTO
					.getDescAgenciaIncorporadoraDepartamento(); //
			estadoMunicIncorpDepto = dependenciaDTO
					.getEstadoMunicipioAgenIncorpDepto();
			codigoPessoaAgencIncorpDepto = dependenciaDTO
					.getCodigoPessoaAgencIncorpDepto();
			numSeqDepIncorpora = dependenciaDTO
					.getNumeroSequencAgencIncorpDepto() != null
					&& !dependenciaDTO.getNumeroSequencAgencIncorpDepto()
							.equals("") ? Integer.valueOf(dependenciaDTO
					.getNumeroSequencAgencIncorpDepto()) : 0;
			descricaoDependencia = dependenciaDTO.getDescricaoDependencia();

			municIncorpDepto = "";
			if (dependenciaDTO.getCodMunicipioAgenIncorpDepto() != null
					&& !dependenciaDTO.getCodMunicipioAgenIncorpDepto().trim()
							.equals("")) {
				Long municipioIncorp = Long.parseLong(dependenciaDTO
						.getCodMunicipioAgenIncorpDepto());
				if (municipioIncorp > 0) {
					municIncorpDepto = municipioIncorp + " - "
							+ dependenciaDTO.getDescMunicipioAgenIncorpDepto();
				}
			}
	...
	public void consultarInformacoesDependencia(
			DependenciasNovasDTO dependenciasNovasDto) {
		try {
			...
			cep = GissUtil.mascaraCep(dependenciaDTO.getCep());
			estado = dependenciaDTO.getEstado();
			indiceComboEmpresaIncorpora = dependenciaDTO
					.getCodigoPessoaAgencIncorpDepto();
			municipio = dependenciaDTO.getMunicipio();
			codDepIncorpora = dependenciaDTO
					.getCodAgenciaIncorporadoraDepartamento() != null
	...
	private void consultarInformacoesDependencia(String parametroNomeMunicipio,
			DependenciasMunicipioDTO dependenciasMunicipioDto) {
		try {
			...
			DependenciaDTO dependenciaDTO = dependenciaImpl
					.consultarInformacoesDependencia(parametroNomeMunicipio,
							dependenciasMunicipioDto
									.getNumeroSequencialDependencia(),
							dependenciasMunicipioDto.getCodigoPessoaJuridica(), dependenciasMunicipioDto.getDataAtualizacao());
			...
			descricaoDependencia = dependenciasMunicipioDto
			...
			numeroSequencAgencIncorpDepto = dependenciaDTO
					.getNumeroSequencAgencIncorpDepto();
			estadoFiltroAgIncorpora = GissUtil
					.procuraCodigoUnidadeFederativaPorSigla(listaDosEstados,
							estadoMunicIncorpDepto);
			codigoMunicipioManter = dependenciaDTO.getCodMunicipio().toString();
			codDepIncorpora = dependenciaDTO
					.getCodAgenciaIncorporadoraDepartamento() != null
					&& !dependenciaDTO.getCodAgenciaIncorporadoraDepartamento()
			...
			Long municipioIncorp = dependenciaDTO
					.getCodMunicipioAgenIncorpDepto() != null
					&& dependenciaDTO.getCodMunicipioAgenIncorpDepto().equals(
							"") ? 0 : Long.parseLong(dependenciaDTO
					.getCodMunicipioAgenIncorpDepto());
			if (municipioIncorp > 0) {
				municIncorpDepto = municipioIncorp + " - "
						+ dependenciaDTO.getDescMunicipioAgenIncorpDepto();
			...
			disabledIndTrib = GissUtil
					.notNull(dependenciaDTO.getTipoDepartamento())
					.substring(0, 1).equals("N");
			...
			BradescoFacesUtils.addInfoModalMessage(p.getMessage(),
					"#{dependenciaBean.voltarManterAlterar}",
					BradescoViewExceptionActionType.ACTION, false);
			flagErro = 1;
		} catch (Exception e) {
			throw new BradescoViewException(e.getMessage(), e, null);
	...
	public void verificarCheck() {
		...
		for (int i = 0; i < codigosSelecionadosTabelaCont.length; i++) {
			...
			for (int j = 0; j < codigosSelecionadosTabela.length; j++) {
				if (codigosSelecionadosTabelaCont[i]
						.equals(codigosSelecionadosTabela[j])) {
					flegado = true;
					break;
				}
			}
	...
	public String prosseguirManterDependencia() {
		if (listaMunicipios != null && listaMunicipios.size() != 0) {
			MunicipioDTO municipioDTO = listaMunicipios.get(new Integer(
					codListaMunicipioSelecionado));

			estado = GissUtil.procuraUnidadeFederativa(listaEstados,
					estadoFiltro);
		...
		// lista os municípios
		listarDependenciasPorMunicipio(
				codDepFiltro != null ? "0" : codigoMunicipio,
				codDepFiltro != null && codDepFiltro != 0 ? codDepFiltro : 0,
				codDepFiltro != null && codDepFiltro != 0 ? indiceComboEmpresaFiltro
						: "0");
		if (dependenciasMunicipios == null) {
			...
			return "lista_dependencias_municipio_parametrizacao";
		}

	}

	private List<MunicipioDTO> listarMunicipiosManterDependencias(String parametroEstado) {

		try {

			List<MunicipioDTO> municipios = dependenciaImpl
					.listarMunicipios(parametroEstado);

			if (municipios != null) {
				listaControleCheckRadio = GissUtil
						.preencherSelectItemsMunicipio(municipios);
	...
	public String carregarListaMunicipiosManter() {
		if (buscaMunicipio.getCodigo() != null
			...
			if (codDepFiltro == null || codDepFiltro == 0) {
				...
				BradescoFacesUtils
						.addInfoModalMessage(
								"Preencher somente um dos argumentos: Estado, Município ou Dependência!",
								false);
			}
		} else if (buscaMunicipio.getDescricao() != null
				&& !buscaMunicipio.getDescricao().equals("")) {
			...
			listarDependenciasPorMunicipio("0", codDepFiltro,
					indiceComboEmpresaFiltro);
			mostraMunicipios = false;
			listaMunicipios.clear();
			estado = GissUtil.procuraCodigoUnidadeFederativaPorSigla(
					listaDosEstados, dependenciasMunicipios.getCodUF())
					+ " - "
					+ dependenciasMunicipios.getDescUF();
			codigoMunicipio = dependenciasMunicipios.getCodMunicipio() != null ? dependenciasMunicipios
					.getCodMunicipio().toString() : "";
			municipio = codigoMunicipio + " - "
					+ dependenciasMunicipios.getDescMunicipio();
			return "lista_dependencias_municipio_parametrizacao";
		} else {
			BradescoFacesUtils
					.addInfoModalMessage(
							"Preencher um dos argumentos: Estado, Município ou Dependência!",
							false);
		}
		return "";
	}
	...
	private void listarDependenciasPorMunicipio(String codMunicipio,
			Integer codDependencia, String codPessoaJuridica) {

		try {
			listaDependenciasMunicipio = new ArrayList<DependenciasMunicipioDTO>();

			dependenciasMunicipios = dependenciaImpl
					.listarDependenciasMunicipio(
							codMunicipio != null && !codMunicipio.equals("") ? codMunicipio
									: "0",
							codDependencia != null ? codDependencia : 0,
							codPessoaJuridica != null
									&& !codPessoaJuridica.equals("") ? codPessoaJuridica
									: "0");
			listaDependenciasMunicipio = dependenciasMunicipios
					.getDepMunicipio();

			listaControleCheckRadio = GissUtil
					.preencherSelectItemsMunicipio(listaDependenciasMunicipio);
		} catch (PdcAdapterFunctionalException p) {
			listaDependenciasMunicipio = new ArrayList<DependenciasMunicipioDTO>();
			BradescoFacesUtils.addInfoModalMessage(p.getMessage(),
					"manterDependencia",
	...
	public String voltarManterDependencia() {
		...
		listaControleCheckRadio = GissUtil
				.preencherSelectItemsMunicipio(listaMunicipios);
		return "manterDependenciaParametrizacao";
	}

	public String voltarResultadoPesquisa() {
		listaControleCheckRadio = GissUtil
				.preencherSelectItemsMunicipio(listaDependenciasMunicipio);
		codListaPesquisar = null;
		resetChaveSegurancaUrl();
		return "lista_dependencias_municipio_parametrizacao";
	}

	private void resetChaveSegurancaUrl() {
		chaveSeguranca = "";
		url = "";
	}
	...
	public String alterarDependenciaManter() {
		...
		DependenciasMunicipioDTO dependenciasMunicipioDTO = listaDependenciasMunicipio
				.get(new Integer(codListaPesquisar));

		codigoPessoaJuridica = dependenciasMunicipioDTO
				.getCodigoPessoaJuridica();
		numeroUnidadeOrganizacional = dependenciasMunicipioDTO
				.getNumeroSequencialDependencia();
		...
		consultarParametrizacaoDep();

		return flagErro == 0 ? "parametrizacao_alteracao" : "";
	}

	public String excluirParametrizacao() {
		flagErro = 0;
		DependenciasMunicipioDTO dependenciasMunicipioDTO = listaDependenciasMunicipio
				.get(new Integer(codListaPesquisar));
		consultarInformacoesDependencia(municipio, dependenciasMunicipioDTO);
		...
		return flagErro == 0 ? "parametrizacao_exclusao" : "";
	}

	public String incluirParametrizacao() {
		flagErro = 0;
		DependenciasMunicipioDTO dependenciasMunicipioDTO = listaDependenciasMunicipio
				.get(new Integer(codListaPesquisar));
		consultarInformacoesDependencia(municipio, dependenciasMunicipioDTO);

		return flagErro == 0 ? "parametrizacao_inclusao" : "";

	}

	private void consultarParametrizacaoDep() {
		DependenciasMunicipioDTO dependenciasMunicipioDTO = listaDependenciasMunicipio
				.get(new Integer(codListaPesquisar));
		String nSeqUnOrg = dependenciasMunicipioDTO
				.getNumeroSequencialDependencia();

		ParametrizacaoDepDTO retorno = parametrizarDependenciaServiceImpl
				.consultarParametrizacao(Long.valueOf(codigoMunicipio),
						Long.valueOf(indiceComboEmpresaFiltro),
						Integer.valueOf(nSeqUnOrg));

		chaveSeguranca = retorno.getChaveSeguranca();
		url = retorno.getUrl();
	}

	public String prosseguirAgenciaDependenciasNovas() {

		if (tipoDepartamento != null && !inscricaoMunicipal.equals("")
				&& !inscricaoMunicipal.equals("0") && !ccm.equals("")
				&& !ccm.equals("0")) {
			...
			BradescoFacesUtils.addInfoModalMessage(
					...
					"carrega_informacoes_dependencias_novas",
					BradescoViewExceptionActionType.ACTION, false);
		}
	...
	public String prosseguirAgenciaIncluirDependencias() {
		...
		if (tipoDepartamento != null && !inscricaoMunicipal.equals("")
			...
			BradescoFacesUtils.addInfoModalMessage(
					"Os campos são obrigatórios.",
					"carrega_informacoes_dependencias_incluir",
					BradescoViewExceptionActionType.ACTION, false);
		}
	...
	public String pesquisarMunicipiodependenciasNovas() {
		...
		BradescoFacesUtils.addInfoModalMessage("Selecione um estado.",
				"carrega_informacoes_dependencias_novas",
				BradescoViewExceptionActionType.ACTION, false);
	...
	public String pesquisarMunicipioManterRecolhimento() {
		...
		BradescoFacesUtils.addInfoModalMessage("Selecione um estado.",
				"manter_alteracao", BradescoViewExceptionActionType.ACTION,
				false);

		return null;
	}

	private List<MunicipioDTO> listarMunicipios(String origem,
		...
		try {
			...
			if (origem.equals("N")) {
				BradescoFacesUtils.addInfoModalMessage(p.getMessage(),
						"prosseguir_agencias_dependencias_novas",
						BradescoViewExceptionActionType.ACTION, false);
				return new ArrayList<MunicipioDTO>();
	...
	public String retornarManterDependenciasAgencia() {
		...
		MunicipioDTO municipioDTO = listaMunicipiosAgIncorpora.get(new Integer(
				codListaPesquisarMunicipioProx));
		codigoMunicipio = municipioDTO.getCodigo();
		codMunicipioIncorp = Long.valueOf(municipioDTO.getCodigo());
		// lista agencias por municipio
	...
	private void listarAgenciasPorMunicipio(String codMunicipio) {
		...
		try {
			listaAgencias = null;
			listaAgencias = dependenciaImpl
					.listarAgenciasMunicipio(codMunicipio);
		} catch (PdcAdapterFunctionalException p) {
			BradescoFacesUtils.addInfoModalMessage(p.getMessage(), false);
	...
	private void listarAgenciasPorMunicipioIncluir(String codMunicipio) {
		...
		try {
			...
			BradescoFacesUtils.addInfoModalMessage(p.getMessage(),
					"prosseguir_agencias_incluir_dependencias",
					BradescoViewExceptionActionType.ACTION, false);
		} catch (Exception e) {
			throw new BradescoViewException(e.getMessage(), e, null);
		}
	...
	public String voltarManterDependenciasMunicipios() {
		listaControleCheckRadio = GissUtil
				.preencherSelectItemsMunicipio(listaMunicipiosAgIncorpora);
		codListaMunicipioSelecionado = null;
		return "lista_municipios_manter_recolhimento";
	}
	...
	public void finalizarExclusaoDependencia(ActionEvent event) {
		...
		DependenciasMunicipioDTO dependMunicipioDTO = listaDependenciasMunicipio
				.get(new Integer(codListaPesquisar));
		codigoPessoaJuridica = dependMunicipioDTO.getCodigoPessoaJuridica()
				.toString();
		numeroUnidadeOrganizacional = dependMunicipioDTO
				.getNumeroSequencialDependencia();
	...
	public String incluirDependenciaNova() {
		...
		try {
			if (indTributavel > 0) {
				...
				dependenciaDTO = dependenciaImpl.incluirDependenciaNova(
						codigoPessoaJuridica, numeroUnidadeOrganizacional,
						codigoMunicipioManter, inscricaoMunicipal,
						indiceComboEmpresaIncorpora,
						String.valueOf(numSeqDepIncorpora), ccm, indTributavel);

				inicializaDependenciasNovas();

				BradescoFacesUtils.addInfoModalMessage(
						dependenciaDTO.getMensagem(),
						"resultadoDependenciasNovas",
						BradescoViewExceptionActionType.ACTION, false);
			} else {
				BradescoFacesUtils.addInfoModalMessage(
	...
	public String incluirDependencia() {
		...
		try {
			if (indTributavel > 0) {
				...
				dependenciaDTO = dependenciaImpl.incluirDependencia(
						codigoPessoaJuridica, numeroUnidadeOrganizacional,
						codigoMunicipioManter, inscricaoMunicipal,
						indiceComboEmpresaIncorpora,
						String.valueOf(numSeqDepIncorpora), ccm, indTributavel);
				...
				BradescoFacesUtils.addInfoModalMessage(
						dependenciaDTO.getMensagem(), "incluirDependencia",
						BradescoViewExceptionActionType.ACTION, false);
			} else {
				BradescoFacesUtils.addInfoModalMessage(
						"Os campos são obrigatórios.",
	...
	public String excluirDependencia() {
		...
		try {
			dependenciaDTO = dependenciaImpl.excluirDependencia(
					codigoPessoaJuridica, numeroUnidadeOrganizacional);
			...
			BradescoFacesUtils.addInfoModalMessage(
					dependenciaDTO.getMensagem(),
					"#{dependenciaBean.prosseguirManterDependencia}",
					BradescoViewExceptionActionType.ACTION, false);

		} catch (PdcAdapterFunctionalException p) {
			BradescoFacesUtils.addInfoModalMessage(p.getMessage(),
					"manter_exclusao", BradescoViewExceptionActionType.ACTION,
					false);
		} catch (Exception e) {
			throw new BradescoViewException(e.getMessage(), e, null);
		}
	...
	public String recarregarLista() {
		listarDependenciasPorMunicipio(
				codDepFiltro != null ? "0" : codigoMunicipio,
				codDepFiltro != null ? codDepFiltro : 0,
				codDepFiltro != null && codDepFiltro != 0 ? indiceComboEmpresaFiltro
						: "0");
		return "lista_dependencias_municipio";
	}
	...
	public String alterarDependencia() {
		...
		try {
			if (indTributavel > 0) {
				...
				dependenciaDTO = dependenciaImpl.alterarDependencia(
						codigoPessoaJuridica, numeroUnidadeOrganizacional,
						codigoMunicipioManter, inscricaoMunicipal,
						indiceComboEmpresaIncorpora,
						String.valueOf(GissUtil.notNull(numSeqDepIncorpora)),
						ccm, indTributavel, dto);
				...
				BradescoFacesUtils.addInfoModalMessage(
						dependenciaDTO.getMensagem(),
						"lista_dependencias_municipio",
						BradescoViewExceptionActionType.ACTION, false);
			} else {
				BradescoFacesUtils.addInfoModalMessage(
						"Os campos são obrigatórios.", "manter_alteracao",
						BradescoViewExceptionActionType.ACTION, false);
			}
		} catch (PdcAdapterFunctionalException p) {
			BradescoFacesUtils.addInfoModalMessage(p.getMessage(),
	...
	private void retornarInformacoesDependencias() {
		...
		AgenciaDTO agenciaDTO = listaAgencias.get(new Integer(
				codListaPesquisarAgencia));
		codDepIncorpora = agenciaDTO.getCodigoDependencia() != null
				&& !agenciaDTO.getCodigoDependencia().equals("") ? Integer
				.valueOf(agenciaDTO.getCodigoDependencia()) : null;
		numSeqDepIncorpora = agenciaDTO.getNumeroSequencialDependencia() != null
				&& !agenciaDTO.getNumeroSequencialDependencia().equals("") ? Integer
				.valueOf(agenciaDTO.getNumeroSequencialDependencia()) : 0;
		indiceComboEmpresaIncorpora = agenciaDTO.getCodigoPessoaJuridica();
		descDepIncorpora = agenciaDTO.getDescricaoDependencia();
		if (codListaPesquisarMunicipioProx != null
				&& !codListaPesquisarMunicipioProx.equals("")
				&& !listaMunicipios.isEmpty()) {
			MunicipioDTO municipioDTO = listaMunicipios.get(new Integer(
					codListaPesquisarMunicipioProx));
			municipioagencia = municipioDTO.getCodigo() + " - "
	...
	public String incluirParametro() {
		...
		String nSeqUnOrg = listaDependenciasMunicipio.get(
				new Integer(codListaPesquisar))
				.getNumeroSequencialDependencia();

		ParametrizacaoDepDTO retorno = parametrizarDependenciaServiceImpl
				.inserirParametrizacao(Long.valueOf(codigoMunicipio),
						Long.valueOf(indiceComboEmpresaFiltro),
						Integer.valueOf(nSeqUnOrg), chaveSeguranca, url);

		BradescoFacesUtils.addInfoModalMessage(retorno.getCodMensagem() + " - "
				+ retorno.getMensagem(), false);

		return voltarResultadoPesquisa();
	}
	...
	public String alterarParametro() {
		...
		ParametrizacaoDepDTO retorno = parametrizarDependenciaServiceImpl
				.alterarParametrizacao(Long.valueOf(codigoMunicipio),
						Long.valueOf(indiceComboEmpresaFiltro),
						Integer.valueOf(nSeqUnOrg), chaveSeguranca, url);
	...
	public String excluirParametro() {
		...
		ParametrizacaoDepDTO retorno = parametrizarDependenciaServiceImpl
				.excluirParametrizacao(Long.valueOf(codigoMunicipio),
						Long.valueOf(indiceComboEmpresaFiltro),
						Integer.valueOf(nSeqUnOrg));
	...
	public String consultarParametrizacao() {
		flagErro = 0;
		DependenciasMunicipioDTO dependenciasMunicipioDTO = listaDependenciasMunicipio
				.get(new Integer(codListaPesquisar));
		String nSeqUnOrg = dependenciasMunicipioDTO
		...
		return flagErro == 0 ? "parametrizacao_consulta" : "";
	...
	public void setListaControleCheckRadio(
			List<SelectItem> listaControleCheckRadio) {
	...
	public void setListaDependenciasNovas(
			List<DependenciasNovasDTO> listaDependenciasNovas) {
	...
	public void setNumeroUnidadeOrganizacional(
			String numeroUnidadeOrganizacional) {
	...
	public void setMunicipioIncorporadoSelecionado(
			MunicipioDTO municipioIncorporadoSelecionado) {
	...
	public void setCodListaPesquisarMunicipioProx(
			String codListaPesquisarMunicipioProx) {
	...
	public void setListaMunicipiosAgIncorpora(
			List<MunicipioDTO> listaMunicipiosAgIncorpora) {
	...
	public void setCodListaMunicipioSelecionado(
			String codListaMunicipioSelecionado) {
	...
	public void setIndiceComboEmpresaIncorpora(
			String indiceComboEmpresaIncorpora) {
	...
	/**
	 * @param selectItemLista
	 *            the selectItemLista to set
	 */
	...
	/**
	 * @param selectItemListaCont
	 *            the selectItemListaCont to set
	 */
	...
	/**
	 * @param codigosSelecionadosTabelaCont
	 *            the codigosSelecionadosTabelaCont to set
	 */
	public void setCodigosSelecionadosTabelaCont(
			Integer[] codigosSelecionadosTabelaCont) {
	...
	/**
	 * @param disabledIndTrib
	 *            the disabledIndTrib to set
	 */
